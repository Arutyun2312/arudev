...
export const FruitView: FC<FruitProps> = ({ fruit, post, postWidth, postHeight, i, trailProgress }) => {
  const shift = (fruit == null ? 0.9 : i * 0.2) - 0.02
  const shineOpacity = useTransform(trailProgress, fruit ? [0.1 + shift, 0.15 + shift, 0.2 + shift] : [0, 1], fruit ? [0, 1, 0] : [0, 1])
  return (
      ...
      <Dialog.Root>
        <Dialog.Trigger asChild>
          <div className='flex h-full w-full items-center justify-center'>
            <motion.div
              className={css(
                'bg-base-2nd fade-in glass-effect h-[calc(100%-16px)] w-[calc(100%-16px)] rounded-md',
                fruit == null ? 'flex items-center justify-center p-3' : 'cursor-pointer p-2',
              )}
              style={{
                boxShadow: useTransform(shineOpacity, (opacity) =>
                  fruit === null ? `0 0 ${30 * opacity + 1}px #22c55e` : `0 0 ${30 * opacity + 1}px #a78bfa`,
                ),
              }}
            >
              {fruit == null ? (
                ...
                <div className='flex h-full items-center'>
                  {fruit.icon && (
                    <Image src={fruit.icon} alt={fruit.company} className='mr-4 h-20 w-20 rounded-lg object-contain' />
                  )}
